// REAL-INPUT drag test, macOS: unlike the e2e suites (synthesized CDP
// events that bypass the OS), this posts genuine HID mouse events via
// cliclick, so the window server's hit-testing — including the
// toolbar's -webkit-app-region: drag — is fully exercised. It drags
// the preview popup's top edge up through the toolbar band with a real
// pressed mouse and asserts the OS never turned it into a window drag
// (the window must not move), the popup survived, and its top rose.
//
// Real input owns the machine's actual cursor, so this only runs in
// CI (or with PT_REAL_INPUT=1 set deliberately).
// Run (CI, macOS): node build-node/test/realDragMac.js   (needs cliclick)
...
function click(...cmds: string[]): void {
  execFileSync('cliclick', cmds, { stdio: 'ignore' });
}
...
async function run(): Promise<void> {
  ...
  try {
    ...
    click(`m:${dx},${dy}`, 'w:150', `dd:${dx},${dy}`, 'w:150');
    for (let i = 1; i <= 6; i++) {
      const y = Math.round(dy + (ty - dy) * (i / 6));
      click(`dm:${tx},${y}`, 'w:80');
    }
    click(`du:${tx},${ty}`);
